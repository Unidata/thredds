...
import static opendap.dap.parsers.ExprParserConstants.FLOATCONST;
import static opendap.dap.parsers.ExprParserConstants.INTCONST;
import static opendap.dap.parsers.ExprParserConstants.STRINGCONST;
...
abstract public class AST
{
    ...
    opendap.dap.parsers.AST root = null;

    void setRoot(opendap.dap.parsers.AST root) {this.root = root;}
    ...
    public AST(List<opendap.dap.parsers.AST> nodes)
    {
    ...
    public void init(CEEvaluator ceEval,
			    ...
			    ClauseFactory clauseFactory,
			    ServerDDS sdds,
			    List<opendap.dap.parsers.AST> nodes)
    {
        this.ceEval = ceEval;
        ...
        this.sdds = sdds;
	this.root = this;
	for(opendap.dap.parsers.AST node: nodes) node.setRoot(this);
    }
...
{
    List<opendap.dap.parsers.ASTprojection> projections = null;
    List<opendap.dap.parsers.ASTclause> selections = null;
...
*/
    ...
    {
        if(projections != null)
            for(opendap.dap.parsers.ASTprojection proj: projections) {
           proj.walk(ceEval);
        } else
	    getCeEval().markAll(true);
        if(selections != null)
            for(opendap.dap.parsers.ASTclause cl: selections) {
...
{
    opendap.dap.parsers.ASTvar var = null;
    opendap.dap.parsers.ASTfcn fcn = null;
...
class ASTfcn extends opendap.dap.parsers.ASTvalue
{
    String fcnname = null;
    List<opendap.dap.parsers.ASTvalue> args = null;
...
*/
	       ...
	       NoSuchFunctionException, NoSuchVariableException
    {
        ...
        if(args != null)
            for(opendap.dap.parsers.ASTvalue arg: args)
...
class ASTvar extends opendap.dap.parsers.ASTvalue
{
    List<opendap.dap.parsers.ASTsegment> segments = new ArrayList<opendap.dap.parsers.ASTsegment>();
...
*/
	...
	throws DAP2ServerSideException, DAP2Exception,
	       NoSuchFunctionException, NoSuchVariableException
    {
        for(opendap.dap.parsers.ASTsegment segment: segments) {
...
{
    String name = null;
    List<opendap.dap.parsers.ASTslice> slices = new ArrayList<opendap.dap.parsers.ASTslice>();
        ...
        if(slices != null && slices.size() > 0) {
            ...
            if(bt instanceof DGrid) {// project the grid and the coordinate variable
                ...
                for(int i=0;i<slices.size();i++) {
                    opendap.dap.parsers.ASTslice slice = slices.get(i);
		            slice.walk(sam,i);
                }
...
{
    opendap.dap.parsers.ASTconstant constant = null;
