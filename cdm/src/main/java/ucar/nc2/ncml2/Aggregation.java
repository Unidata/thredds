...
public abstract class Aggregation implements ProxyReader2 {
  ...
  static protected org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(ucar.nc2.ncml.Aggregation.class);
  ...
  public synchronized boolean syncExtend(boolean force) throws IOException {
    ...
    if ((getType() == Type.FORECAST_MODEL_COLLECTION) || (getType() == Type.FORECAST_MODEL_SINGLE)) {
      //ucar.unidata.io.RandomAccessFile.setDebugAccess( true);
      syncDataset(null);
      //ucar.unidata.io.RandomAccessFile.setDebugAccess( false);
