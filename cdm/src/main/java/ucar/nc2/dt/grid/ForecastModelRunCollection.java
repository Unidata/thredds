...
public class ForecastModelRunCollection {
   ...
   * @throws IOException
   */
  ForecastModelRunCollection(String fmrcDefinitionDir, String name) throws IOException {
   ...
   */
  void addRun(ForecastModelRun fmr) {
    ...
    for (int i = 0; i < timeCoords.size(); i++) {
      ...
      ForecastModelRun.TimeCoord tcUse = findTime( tc);
      if (tcUse == null) {
      ...
      for (int j = 0; j < varList.size(); j++) {
        ForecastModelRun.Grid grid = (ForecastModelRun.Grid) varList.get(j);
  ...
  static class Run implements Comparable {
    ...
    Run(Date runTime, ForecastModelRun.TimeCoord tc) {
      this.runTime = runTime;
      this.tc = tc;
  ...
  private ForecastModelRun.TimeCoord findTime(ForecastModelRun.TimeCoord want) {
    for (int i = 0; i < timeCoords.size(); i++) {
      ForecastModelRun.TimeCoord  tc = (ForecastModelRun.TimeCoord) timeCoords.get(i);
  ...
  private ForecastModelRun.VertCoord findVertCoord(ForecastModelRun.VertCoord want) {
    for (int i = 0; i < vertCoords.size(); i++) {
      ForecastModelRun.VertCoord  vc = (ForecastModelRun.VertCoord) vertCoords.get(i);
  ...
  class UberGrid implements Comparable {
    ...
    ForecastModelRun.VertCoord vertCoordUnion = null;
    ...
    void addRun(Run run, ForecastModelRun.Grid grid) {
      ForecastModelRun.TimeCoord xtc = (expectedSeq == null) ? null : expectedSeq.findTimeCoordByRuntime( run.runTime);
    ...
    void finish() {
      ...
      ForecastModelRun.VertCoord vc_union = null;
      for (int i = 0; i < runs.size(); i++) {
        RunExpected rune = (RunExpected) runs.get(i);
        ForecastModelRun.VertCoord vc = rune.grid.vc;
        if (vc == null) continue;
        if (vc_union == null)
          vc_union = new ForecastModelRun.VertCoord(vc);
     ...
     */
    public void normalize(ForecastModelRun.VertCoord result, List vcList) {
      // get all values into a HashSet of Double
      HashSet valueSet = new HashSet();
      addValues( valueSet, result.getValues());
      for (int i = 0; i < vcList.size(); i++) {
        ForecastModelRun.VertCoord vc = (ForecastModelRun.VertCoord) vcList.get(i);
  ...
  class RunExpected implements Comparable {
    ...
    ForecastModelRun.Grid grid;           //  grid containing actual vert coord
    ForecastModelRun.TimeCoord expected;  // expected time coord
    FmrcDefinition.Grid expectedGrid;     // expected grid

     RunExpected(Run run, ForecastModelRun.TimeCoord expected, ForecastModelRun.Grid grid, FmrcDefinition.Grid expectedGrid) {
       this.run = run;
   ...
   * @throws Exception
   */
  public static ForecastModelRunCollection make(String fmrcDefinitionPath, String collectionName,
          ucar.nc2.util.DiskCache2 fmr_cache, String dirName, String suffix, int mode) throws Exception {

    long startTime = System.currentTimeMillis();
    ForecastModelRunCollection fmrCollection = new ForecastModelRunCollection(fmrcDefinitionPath, collectionName);
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      ForecastModelRun fmr = ForecastModelRun.open(fmr_cache, file.getPath(), mode);
      if (null != fmr)
  ...
  public static void main(String args[]) throws Exception {
    ...
    ForecastModelRunCollection fmrc = make("C:/data/grib/"+dir+"/", "NCEP-NAM-CONUS_12km", null, "C:/data/grib/"+dir, "grib2",
            ForecastModelRun.OPEN_NORMAL);
