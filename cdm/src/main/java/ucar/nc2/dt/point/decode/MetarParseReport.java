...
//	next if( /\d4,6Z.*\d4,6Z/ ) ;
        if (DecoderPatterns.p_bad_date.matcher(input).find())
            return null;
//	next if( /^\n/ ) ;
        if (DecoderPatterns.p_B_CR.matcher(input).find())
            return null;
...
//	$rep_type = $1 if( s#^(METAR|SPECI|TESTM|TESTS) ## ) ;
        m = DecoderPatterns.p_B_metar.matcher(input);
        if (m.find()) {
...
//	next unless( $stn_name ) ;
        m = DecoderPatterns.p_station.matcher(input);
        if (m.find()) {
            metar.put("Station", m.group(1));
            //input = DecoderPatterns.p_B_W4.matcher( input ).replaceFirst( "" );
            input = m.replaceFirst("");
        } else {
            return null;
        }
        // get day hour minute
//	if( s#^(\d2)(\d2)(\d2)Z## )
        m = DecoderPatterns.p_ddhhmmZ.matcher(input);
        if (m.find()) {
//		$rday = $1 ;
...
//		$rmin = $3 ;
        ...
        // skip NIL reports
//	next if( s#NIL\s*## ) ;
        if (DecoderPatterns.p_NILS.matcher(input).find())
            return null;

        m = DecoderPatterns.p_COR.matcher(input);
        if (m.find()) {
            input = m.replaceFirst("");
        }

//	$AUTO = 1 if( s#AUTO\s+## ) ;
        m = DecoderPatterns.p_AUTOS.matcher(input);
        if (m.find()) {
            metar.put("AUTOS", "1");
            input = m.replaceFirst("");
        }
...
//	if( s#(E|W|N|S)?(\d3|VRB)(\d2,3)(G)?(\d2,3)?(KMH|KT|MPS|MPH)\s+## )
        m = DecoderPatterns.p_wind_direction_speed.matcher(input);
        if (m.find()) {
//		if( $2 eq "VRB" )
...
//	if( s#^(\d3)V(\d3)\s+## )
        m = DecoderPatterns.p_min_max_wind_dir.matcher(input);
        if (m.find()) {
//		$DIRmin = $1 ;
...
//		$DIRmax = $2 ;
        ...
        // get visibility
//	$prevail_VIS_SM = 10.0 if( s#^9999NDV\s*## ) ;
        m = DecoderPatterns.p_9999NDVS.matcher(input);
        if (m.find()) {
            metar.put("Visibility_SM", "10.0");
            input = m.replaceFirst("");
        }

        // some reports use a place holder for visibility
        //		s#9999\s+## ;
        input = DecoderPatterns.p_9999S.matcher(input).replaceFirst("");

//	$prevail_VIS_SM = 0.0 if( s#^M1/4SM\s+|<1/4SM\s+## ) ;
        m = DecoderPatterns.p_visibilitySM.matcher(input);
        if (m.find()) {
...
//	$prevail_VIS_KM = 0.0 if( s#^M1/4KM\s+|<1/4KM\s+## ) ;
        m = DecoderPatterns.p_visibilityKM.matcher(input);
        if (m.find()) {
...
//	$plus_VIS_SM = 1 if( s#^P(\d1,3)SM\s+#$1SM # ) ;
        m = DecoderPatterns.p_B_P.matcher(input);
        if (m.find()) {
...
//	if( s#^(\d1,4) (\d1,3)/(\d1,3)(SM|KM)\s+## )
        m = DecoderPatterns.p_visibility1.matcher(input);
        if (m.find()) {
...
//	 elsif( s#^(\d1,3)/(\d1,3)(KM|SM)\s+## )
            ...
            m = DecoderPatterns.p_visibility2.matcher(input);
            if (m.find()) {
                var1 = Float.parseFloat(m.group(1));
                var2 = Float.parseFloat(m.group(2));
                var1 = var1 / var2;
...
//		 elsif( s#^(\d1,4)(KM|SM)\s+## )
                ...
                m = DecoderPatterns.p_visibility3.matcher(input);
                if (m.find()) {
...
//		 	elsif( s#^(\d4)((NE)|(NW)|(SE)|(SW)|(N)|(S)|(E)|(W))\s+## )
                    ...
                    m = DecoderPatterns.p_visibility_direction.matcher(input);
                    if (m.find()) {
//					$prevail_VIS_M = $1 ;
...
//	$CAVOK = 1 if( s#CAVOK\s+## ) ;
        m = DecoderPatterns.p_CAVOKS.matcher(input);
        if (m.find()) {
...
//	$RVRNO = 1 if( s#RVRNO\s+## ) ;
        m = DecoderPatterns.p_RVRNOS.matcher(input);
        if (m.find()) {
            metar.put("RVRNOS", "1");
            input = m.replaceFirst("");
        }
...
//		if( s#R(\d2)(R|L|C)?/(M|P)?(\d1,4)V?(M|P)?(\d1,4)?(FT|N|D)?\s+## )
            m = DecoderPatterns.p_runway.matcher(input);
            if (m.find()) {
//			$RV_designator[ $i ] = "$1$2" ;
...
//		if( s#^(\+|-|VC|PR)?(MI|BC|DR|BL|SH|TS|FZ)?(DZ|RA|SN|SG|IC|PE|PL|GR|GS|UP)?(BR|FG|FU|VA|DU|SA|HZ|PY)?(PO|SQ|FC|SS|DS)?\s+## )
            m = DecoderPatterns.p_weather.matcher(input);
            if (m.find()) {
...
// 	Interpret cloud conditions   Variables need to be stored
        for (int i = 0; i < 3; i++) {
           m = DecoderPatterns.p_cloud_percent.matcher(input);
           if (m.find()) {
...
//	$cloud_type[ 0 ] = $1 if( s#(CLR|SKC)\s+## ) ;
        m = DecoderPatterns.p_CLR_or_SKC.matcher(input);
        if (m.find()) {
            metar.put("Cloud_Type", "1");
            input = m.replaceFirst("");
        }
//	$vert_VIS = cloud_hgt2_meters( $1 ) if( s#^VV(\d3)\s+## ) ;
        m = DecoderPatterns.p_vertical_VIS.matcher(input);
        if (m.find()) {
...
//		if( s#^(\+|-)?(OVC|SCT|FEW|BKN)(\d3)(\w1,3)?\s+## )
            m = DecoderPatterns.p_cloud_cover.matcher(input);
            if (m.find()) {
...
//	if( s#^(M)?(\d2)/(M)?(\d2)?\s+## )
        m = DecoderPatterns.p_temperature.matcher(input);
        if (m.find()) {
//		$T = $2 ;
            String T = m.group(2);
//		$T *= -1 if( $1 ) ;
...
//	if( s#^(A|Q)(\d4\.?\d?)\s+## )
        m = DecoderPatterns.p_altimeter.matcher(input);
        if (m.find()) {
//		if( $1 eq "A" )
...
//	$NOSIG = 1 if( s#NOSIG## ) ;
        m = DecoderPatterns.p_NOSIG.matcher(input);
        if (m.find()) {
...
// 	check for remarks or done
        m = DecoderPatterns.p_REMARKS.matcher(input);
        if (m.find()) {
            input = m.replaceFirst("");
        } else {
            if(input.length() > 0)
...
//		$AUTOindicator = padstr( $1, 4 )
        m = DecoderPatterns.p_automatic_report.matcher(input);
        if (m.find()) {
...
//	check if no more info in report
        if (input.equals("") || DecoderPatterns.p_spaces.matcher(input).matches()) {
            return metar;
        }

        // Sea-Level presure
        //$SLPNO = 1 if( s#SLPNO\s+## ) ;
        m = DecoderPatterns.p_SLPNO.matcher(input);
        if (m.find()) {
            metar.put("SLPNO", "1");
            input = m.replaceFirst("");
        }
        ...
        //if( s#SLP\s?(\d3)\s+## )
        m = DecoderPatterns.p_SLP.matcher(input);
        if (m.find()) {
            float slp;
            //	if( $1 >= 550 )
...
//	if( s#T(0|1)(\d3)(0|1)?(\d3)?\s+## )
        m = DecoderPatterns.p_temperature_tenths.matcher(input);
        if (m.find()) {
//		if( $1 == 0 )
//			$T_tenths = 0.1 * $2 ;
//	 	else
...
//	$PRECIP_hourly = $1 / 100 if( s#P ?(\d1,5)\s+## ) ;
        m = DecoderPatterns.p_hourly_precip.matcher(input);
        if (m.find()) {
...
//	check if no more info in report
        ...
        m = DecoderPatterns.p_PWINO.matcher(input);
        if (m.find()) {
            metar.put("PWINO", "1");
            input = m.replaceFirst("");
        }
...
//	$TSNO = 1 if( s#TSNO\s+## ) ;
        m = DecoderPatterns.p_TSNO.matcher(input);
        if (m.find()) {
            metar.put("TSNO", "1");
            input = m.replaceFirst("");
        }
...
//	if( s#(TORNADO\w0,2|WATERSPOUTS*|FUNNEL CLOUDS*)\s+## )
        m = DecoderPatterns.p_tornado.matcher(input);
        if (m.find()) {
//		$TornadicType = padstr( $1, 15 ) ;
            metar.put("TornadicType", m.group(1));
            input = m.replaceFirst("");
//		if( s#(B|E)(\d\d)(\d\d)?\s+## )
            m = DecoderPatterns.p_tornadoTime.matcher(input);
            if (m.find()) {
//			if( $1 eq "B" )
...
//			if( s#^(DSNT|VCY STN|VC STN|VCY|VC)\s+## ) ;
            m = DecoderPatterns.p_tornadoLocation.matcher(input);
            if (m.find()) {
...
//			if( s#^(NE|NW|SE|SW|N|S|E|W)\s+## ) ;
            m = DecoderPatterns.p_tornadoDirection.matcher(input);
            if (m.find()) {
...
//	if( s#PK WND (\d3)(\d1,3)/(\d\d)?(\d\d)\s+## )
        m = DecoderPatterns.p_peakWind.matcher(input);
        if (m.find()) {
//		$PKWND_dir = $1 ;
...
//	if( s#WSHFT (\d\d)?(\d\d)\s+## )
        m = DecoderPatterns.p_windShift.matcher(input);
        if (m.find()) {
...
//	$Wshft_FROPA = 1 if( s#FROPA\s+## ) ;
        m = DecoderPatterns.p_FROPA.matcher(input);
        if (m.find()) {
...
//	$VIS_TWR = $2 + ( $3 / $4 ) ;
        m = DecoderPatterns.p_towerVisibility1.matcher(input);
        if (m.find()) {
...
//		$VIS_TWR = ( $2 / $3 ) ;
            ...
            m = DecoderPatterns.p_towerVisibility2.matcher(input);
            if (m.find()) {
                var1 = Float.parseFloat(m.group(2));
                var2 = Float.parseFloat(m.group(3));
                var1 = var1 / var2;
...
//			$VIS_TWR = $2 ;
                ...
                m = DecoderPatterns.p_towerVisibility3.matcher(input);
                if (m.find()) {
...
//	$VIS_SFC = $2 + ( $3 / $4 ) ;
        m = DecoderPatterns.p_surfaceVisibility1.matcher(input);
        if (m.find()) {
            var1 = Float.parseFloat(m.group(2));
            var2 = Float.parseFloat(m.group(3));
            var3 = Float.parseFloat(m.group(4));
...
//			$VIS_SFC = ( $2 / $3 ) ;
            ...
            m = DecoderPatterns.p_surfaceVisibility2.matcher(input);
            if (m.find()) {
                var1 = Float.parseFloat(m.group(2));
                var2 = Float.parseFloat(m.group(3));
                var1 = var1 / var2;
...
//			$VIS_SFC = $2 ;
                ...
                m = DecoderPatterns.p_surfaceVisibility3.matcher(input);
                if (m.find()) {
...
//		$VISmin = $2 + ( $3 / $4 ) ;
//		$VISmax = $5 + ( $6 / $7 ) ;
        m = DecoderPatterns.p_variableVisibility1.matcher(input);
        if (m.find()) {
            var1 = Float.parseFloat(m.group(2));
            var2 = Float.parseFloat(m.group(3));
            var3 = Float.parseFloat(m.group(4));
...
//		$VISmax = $3 + ( $4 / $5 ) ;
            ...
            m = DecoderPatterns.p_variableVisibility2.matcher(input);
            if (m.find()) {
...
//			$VISmax = $4 + ( $5 / $6 ) ;
                ...
                m = DecoderPatterns.p_variableVisibility3.matcher(input);
                if (m.find()) {
                    var1 = Float.parseFloat(m.group(2));
                    var2 = Float.parseFloat(m.group(3));
                    var1 = var1 / var2;
...
//				$VISmax = $5 ;
                    ...
                    m = DecoderPatterns.p_variableVisibility4.matcher(input);
                    if (m.find()) {
                        var1 = Float.parseFloat(m.group(2));
                        var2 = Float.parseFloat(m.group(3));
                        var3 = Float.parseFloat(m.group(4));
...
//					$VISmax = $3 ;
                        ...
                        m = DecoderPatterns.p_variableVisibility5.matcher(input);
                        if (m.find()) {
                            metar.put("Variable_Visibility_Min", m.group(2));
...
//						$VISmax = $4 ;
                            ...
                            m = DecoderPatterns.p_variableVisibility6.matcher(input);
                            if (m.find()) {
                                var1 = Float.parseFloat(m.group(2));
                                var2 = Float.parseFloat(m.group(3));
                                var1 = var1 / var2;
...
//		$VIS_2ndSite_LOC = padstr( $5, 10 ) ;
        m = DecoderPatterns.p_Visibility2ndSite1.matcher(input);
        if (m.find()) {
            var1 = Float.parseFloat(m.group(2));
            var2 = Float.parseFloat(m.group(3));
            var3 = Float.parseFloat(m.group(4));
...
//		$VIS_2ndSite_LOC = padstr( $3, 10 ) ;
            ...
            m = DecoderPatterns.p_Visibility2ndSite2.matcher(input);
            if (m.find()) {
...
//			$VIS_2ndSite_LOC = padstr( $4, 10 ) ;
                ...
                m = DecoderPatterns.p_Visibility2ndSite3.matcher(input);
                if (m.find()) {
                    var1 = Float.parseFloat(m.group(2));
                    var2 = Float.parseFloat(m.group(3));
                    var1 = var1 / var2;
...
//	if( s#(OCNL|FRQ|CNS) LTG\s?(CG|IC|CC|CA)\s?(DSNT|AP|VCY STN|VCNTY STN)?\s?(NE|NW|SE|SW|N|S|E|W)?\s+## )
        m = DecoderPatterns.p_Lightning.matcher(input);
        if (m.find()) {
...
//		$Ceiling_min = $1 ;
//		$Ceiling_max = $2 ;
        m = DecoderPatterns.p_CIG.matcher(input);
        if (m.find()) {
...
//		$CIG_2ndSite_LOC = $2 ;
        m = DecoderPatterns.p_CIG_RY.matcher(input);
        if (m.find()) {
...
//	$PRESFR = 1 if( s#PRESFR/?\s+## ) ;
        m = DecoderPatterns.p_PRESFR.matcher(input);
        if (m.find()) {
...
//	$PRESRR = 1 if( s#PRESRR/?\s+## ) ;
        m = DecoderPatterns.p_PRESRR.matcher(input);
        if (m.find()) {
...
//		$SectorVIS = $3 + ( $4 / $5 ) ;
        m = DecoderPatterns.p_sectorVisibility1.matcher(input);
        if (m.find()) {
...
//		$SectorVIS = ( $3 / $4 ) ;
            ...
            m = DecoderPatterns.p_sectorVisibility2.matcher(input);
            if (m.find()) {
                metar.put("Sector_Visibility_Direction", m.group(2));
                var1 = Float.parseFloat(m.group(3));
                var2 = Float.parseFloat(m.group(4));
...
//			$SectorVIS = $3 ;
                ...
                m = DecoderPatterns.p_sectorVisibility3.matcher(input);
                if (m.find()) {
                    metar.put("Sector_Visibility_Direction", m.group(2));
...
//		$GR = 1 ;
//		$GRsize = 1 / 8 ;
        m = DecoderPatterns.p_GR1.matcher(input);
        if (m.find()) {
...
//		$GRsize = $1 + ( $2 / $3 ) ;
            ...
            m = DecoderPatterns.p_GR2.matcher(input);
            if (m.find()) {
                metar.put("Hailstone_Activity", "1");
                var1 = Float.parseFloat(m.group(1));
                var2 = Float.parseFloat(m.group(2));
...
//			$GRsize = ( $1 / $2 ) ;
                ...
                m = DecoderPatterns.p_GR3.matcher(input);
                if (m.find()) {
                    metar.put("Hailstone_Activity", "1");
                    var1 = Float.parseFloat(m.group(1));
                    var2 = Float.parseFloat(m.group(2));
...
//				$GRsize = $1 ;
                    ...
                    m = DecoderPatterns.p_GR4.matcher(input);
                    if (m.find()) {
                        metar.put("Hailstone_Activity", "1");
...
//	$GR = 1 if( s#GS\s+## ) ;
        m = DecoderPatterns.p_GR.matcher(input);
        if (m.find()) {
            metar.put("Hailstone_Activity", "1");
            input = m.replaceFirst("");
        }
...
//		$VIRGAdir = padstr( $2, 2 ) if( $2 ) ;
        m = DecoderPatterns.p_VIRGA.matcher(input);
        if (m.find()) {
...
//		$OctsSkyObscured = $6 ;
        m = DecoderPatterns.p_obscuring.matcher(input);
        if (m.find()) {
            String tmp = "";
            if (m.group(1) != null) {
                tmp = m.group(1);
...
//	$CIGNO = 1 if( s#CIGNO\s+## ) ;
        m = DecoderPatterns.p_CIGNO.matcher(input);
        if (m.find()) {
            metar.put("CIGNO", "1");
            input = m.replaceFirst("");
        }
...
//		 else
//			$Ceiling = $2 * 100 ;
        m = DecoderPatterns.p_CIG_EST.matcher(input);
        if (m.find()) {
...
//		$VrbSkyAbove = $3 ;
        m = DecoderPatterns.p_variableSky.matcher(input);
        if (m.find()) {
...
//			if( s#^(NE|NW|SE|SW|N|S|E|W)(\-| MOV )?(NE|NW|SE|SW|N|S|E|W)?/?\s+## ) ;
        m = DecoderPatterns.p_significantCloud.matcher(input);
        if (m.find()) {
            metar.put("Significant_Cloud", m.group(1));
            input = m.replaceFirst("");
            m = DecoderPatterns.p_significantCloud1.matcher(input);
            if (m.find()) {
                metar.put("Significant_Cloud_Vicinity", m.group(1));
                input = m.replaceFirst("");
                m = DecoderPatterns.p_significantCloud2.matcher(input);
                if (m.find()) {
...
//
        m = DecoderPatterns.p_obscuringPhen.matcher(input);
        if (m.find()) {
            String tmp = "";
            if (m.group(1) != null) {
                tmp = m.group(1);
...
//	$ACFTMSHP = 1 if( s#\(?ACFT\s?MSHP\)?\s+## ) ;
        m = DecoderPatterns.p_ACFT.matcher(input);
        if (m.find()) {
...
//	$NOSPECI = 1 if( s#NOSPECI\s+## ) ;
        m = DecoderPatterns.p_NOSPECI.matcher(input);
        if (m.find()) {
...
//	$FIRST = 1 if( s#FIRST\s+## ) ;
        m = DecoderPatterns.p_FIRST.matcher(input);
        if (m.find()) {
...
//	$LAST = 1 if( s#LAST\s+## ) ;
        m = DecoderPatterns.p_LAST.matcher(input);
        if (m.find()) {
...
//		$Cloud_medium = $2 ;
//		$Cloud_high = $3 ;
        m = DecoderPatterns.p_cloud_height.matcher(input);
        if (m.find()) {
...
//		$SNINCR = $1 ;
//		$SNINCR_TotalDepth = $2 ;
        m = DecoderPatterns.p_SNINCR.matcher(input);
        if (m.find()) {
...
//	if( s#4/(\d1,3)\s+# # )
//		$SN_depth = $1 ;
        m = DecoderPatterns.p_snowDepth.matcher(input);
        if (m.find()) {
...
//	$SN_waterequiv = $1 / 10 if( s# 933(\d3)\s+# # ) ;
        m = DecoderPatterns.p_waterEquiv.matcher(input);
        if (m.find()) {
...
//		 else
//			$SunShineDur = $1 ;
        m = DecoderPatterns.p_sunShine.matcher(input);
        if (m.find()) {
            if( m.group(1).equals("///") ) {
...
//		$PRECIP_amt = $1 / 100 if( $1 ne "////" ) ;
        m = DecoderPatterns.p_precipitation.matcher(input);
        if (m.find()) {
            if( ! m.group(1).equals("////") ) {
...
//		$PRECIP_24_amt = $1 / 100 if( $1 ne "////" ) ;
        m = DecoderPatterns.p_precipitation24.matcher(input);
        if (m.find()) {
            if( ! m.group(1).equals("////") ) {
...
//		$Tmax *= -1.0 if( $1 == 1 ) ;
//
        m = DecoderPatterns.p_maxTemperature.matcher(input);
        if (m.find()) {
...
//		$Tmin = $2 / 10 if( $2 ne "///" ) ;
//		$Tmin *= -1.0 if( $1 == 1 ) ;
        m = DecoderPatterns.p_minTemperature.matcher(input);
        if (m.find()) {
            if( ! m.group(2).equals("////") && m.group(1).equals("1") ) {
...
//		$Tmin24 = $4 / 10 if( $4 ne "///" ) ;
//		$Tmin24 *= -1.0 if( $3 == 1 ) ;
        m = DecoderPatterns.p_maxMinTemp24.matcher(input);
        if (m.find()) {
            if( ! m.group(2).equals("////") && m.group(1).equals("1") ) {
...
//		$Ptend = $2 / 10 if( $2 ne "///" ) ;
//
        m = DecoderPatterns.p_pressureTendency.matcher(input);
        if (m.find()) {
...
//	$FZRANO = 1 if( s#FZRANO\s+## ) ;
        m = DecoderPatterns.p_FZRANO.matcher(input);
        if (m.find()) {
...
//	$PNO = 1 if( s#PNO\s+## ) ;
        m = DecoderPatterns.p_PNO.matcher(input);
        if (m.find()) {
...
//	$maintIndicator = 1 if( s#\$\s+## ) ;
        m = DecoderPatterns.p_maintenace.matcher(input);
        if (m.find()) {
...
//	    if( s#(\+|-|VC|PR)?(MI|BC|DR|BL|SH|TS|FZ)?(DZ|RA|SN|SG|IC|PE|PL|GR|GS|UP)?(BR|FG|FU|VA|DU|SA|HZ|PY)?(PO|SQ|FC|SS|DS)?B(\d2,4)E(\d2,4)\s+## )
            m = DecoderPatterns.p_recentWeather.matcher(input);
            if (m.find()) {
...
//	    elsif( s#(\+|-|VC|PR)?(MI|BC|DR|BL|SH|TS|FZ)?(DZ|RA|SN|SG|IC|PE|PL|GR|GS|UP)?(BR|FG|FU|VA|DU|SA|HZ|PY)?(PO|SQ|FC|SS|DS)?(B|E)(\d2,4)\s+## )
                ...
                m = DecoderPatterns.p_recentWeather1.matcher(input);
                if (m.find()) {
//		$Recent_WX[ $i ] = padstr( "$1$2$3$4$5", 8 ) ;
                    String tmp = "";
                    if (m.group(1) != null) {
...
//	$PlainText = padstr( $1, 128 ) if( s#(\w.*)## ) ;
        m = DecoderPatterns.p_plainText.matcher(input);
        if (m.find()) {
...
//		$meters = 30 * $height ;
  ...
  private static void doOneFile(String location) throws IOException, InvalidRangeException {
     MetarParseReport func = new MetarParseReport();

    NetcdfFile ncfile = NetcdfFile.open(location);
    Variable v = ncfile.findVariable("report");
    ArrayChar data = (ArrayChar) v.read();

    ArrayChar.StringIterator iter = data.getStringIterator();
    int count = iter.getNumElems();
    System.out.println(location +" has "+count+" metar reports");
    long start = System.currentTimeMillis();

    while (iter.hasNext()) {
      String text = iter.next();
      try {
        LinkedHashMap metar = func.parseReport(text);
      } catch (Exception e) {
    ...
    double tookPer = took/ (double) count;
    System.out.println("that took "+took+" msec = "+tookPer+" msec/report");
  }
...
/*
      ...
      //String report = "KWRB 311155Z 00000KT 1SM FG SKC 13/13 A3027 RMK VIS N 1/2 FG  BKN003 SLP251 51015";

      //String report = "METAR BGQQ 310850Z AUTO 32001KT 9999NDV ///049/// ///055/// M09/M18 Q1021 RMK /// ///";

      //String report = "METAR BGUK 310350Z AUTO 10005KT 9999NDV ///017/// ///021/// ///024/// M02/M06 Q1020 RMK /// /// ///";
