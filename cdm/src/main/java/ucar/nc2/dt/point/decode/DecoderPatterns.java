...
public class DecoderPatterns {

    public static final Pattern p_9999S = Pattern.compile("9999\\s*");

    public static final Pattern p_9999NDVS = Pattern.compile("9999NDV\\s*");
    ...
    public static final Pattern p_cloud_percent = Pattern.compile("///(\\d{3})///\\s+");
    ...
    public static final Pattern p_obscuringPhen = Pattern.compile("(VC|PR)?(MI|BC|DR|BL|SH|TS|FZ)?(DZ|RA|SN|SG|IC|PE|PL|GR|GS|UP)?(BR|FG|FU|VA|DU|SA|HZ|PY)?(PO|SQ|FC|SS|DS)? (FEW|SCT|BKN|OVC)(\\d{3})\\s+" ); 
