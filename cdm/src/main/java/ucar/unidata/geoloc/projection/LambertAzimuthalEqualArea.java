...
public class LambertAzimuthalEqualArea extends ProjectionImpl {
    ...
    public LambertAzimuthalEqualArea(double lat0, double lon0) {
        this(lat0, lon0, 0.0, 0.0, null, EARTH_RADIUS);
    }

    /**
     * Construct a LambertAzimuthalEqualArea Projection, two standard parellels.
     ...
     * @param units   units of the false_easting, false_northing
     *                (ignored if zero), must be udunits convertible to km.
    ...
    /**
     * Get the conversion factor from meters to km
     *
     * @param inputUnitString input string
     * @param outputUnitString output string
     *
     ...
     */
    public Object clone() {
        LambertAzimuthalEqualArea cl =
            (LambertAzimuthalEqualArea) super.clone();
