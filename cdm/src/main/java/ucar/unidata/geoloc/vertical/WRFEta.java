...
public class WRFEta extends VerticalTransformImpl {
   ...
   */
  public WRFEta(NetcdfDataset ds, Dimension timeDim, VerticalCT vCT) {

    super(timeDim);

    Parameter param = vCT.findParameterIgnoreCase(IsStaggeredX);
    isXStag = (param != null) && Boolean.valueOf(param.getStringValue()).booleanValue();
    param = vCT.findParameterIgnoreCase(IsStaggeredY);
    isYStag = (param != null) && Boolean.valueOf(param.getStringValue()).booleanValue();
    param = vCT.findParameterIgnoreCase(IsStaggeredZ);
    isZStag = (param != null) && Boolean.valueOf(param.getStringValue()).booleanValue();
    ...
    if (isZStag) {
      ...
      pertVarName = vCT.findParameterIgnoreCase(PerturbationGeopotentialVariable).getStringValue();
      baseVarName = vCT.findParameterIgnoreCase(BaseGeopotentialVariable).getStringValue();
      ...
      pertVarName = vCT.findParameterIgnoreCase(PerturbationPressureVariable).getStringValue();
      baseVarName = vCT.findParameterIgnoreCase(BasePressureVariable).getStringValue();
      units = "Pa";  //P and PB are in Pascals  //ADD:safe assumption? grab unit attribute?
    }

    pertVar = ds.findStandardVariable(pertVarName);
    baseVar = ds.findStandardVariable(baseVarName);
      ...
      for (int i = 0; i < ((dimIndex == 0)
        ...
        for (int j = 0; j < ((dimIndex == 1)
          ...
          for (int k = 0; k < ((dimIndex == 2)
            ...
            origin[2] = k;
            IndexIterator it =
                    array.section(origin, eshape).getIndexIterator();
            for (int l = 0; l < n; l++) {
