...
@Service
public class NcssShowDatasetInfoImpl implements NcssShowFeatureDatasetInfo,
		ServletContextAware {

	static private final Logger log = LoggerFactory
			.getLogger(NcssShowDatasetInfoImpl.class);

	private ServletContext servletContext;

	public String showForm(FeatureDataset fd, String datasetUrlPath,
			boolean wantXml, boolean isPoint) {
		...
		switch (ft) {
		case GRID:
			strResponse = showGridForm((GridDataset) fd, datasetUrlPath,
					wantXml, isPoint);
			break;
			
		case STATION:
			strResponse = showStationsForm((FeatureDatasetPoint) fd, datasetUrlPath, wantXml);
			break;
	...
	private String showStationsForm(FeatureDatasetPoint fp, String datasetUrlPath, boolean wantXml){
	...
	private String showGridForm(GridDataset gds, String datsetUrlPath,
			boolean wantXml, boolean isPoint) {

		boolean isNetcdf4Availalbe = FormatsAvailabilityService
		...
		if (wantXml) {
			...
			try {
				...
				InputStream xslt = getXSLT(isPoint ? "/WEB-INF/xsl/ncssGridAsPoint.xsl"
						: "/WEB-INF/xsl/ncssGrid.xsl");
				...
				Transformer xslTransformer = TransformerFactory.newInstance()
						.newTransformer(new StreamSource(xslt));
				String context = servletContext.getContextPath();
				xslTransformer.setParameter("tdsContext", context);

				xslTransformer.setParameter("gridWKT",
						writer.getDatasetBoundariesWKT());
	...
	private InputStream getXSLT(String xslName) throws IOException {
		ServletContextResource r = new ServletContextResource(servletContext,
				xslName);
	...
	private void addNetcdf4Format(Document datasetDescriptionDoc,
