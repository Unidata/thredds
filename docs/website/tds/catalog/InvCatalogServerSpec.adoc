...
The *<<../tutorial/NcML.htm,ncml:netcdf>>* element is specific to server side catalogs, along with the
...
<<../tutorial/NcML.htm,modifies the dataset with NcML>>. For
<<#datasetScan,datasetScan>>, it modifies all contained datasets.
...
description, <<#namer_Element,above>>, for more on regular expressions
...
* <<#datasetScan,datasetScan>>: creates nested catalogs by scanning directories.
* <<#datasetRoot,datasetRoot>>: associates a local disk directory with a URL path +
* <<#filter,filter>>
* <<#namer,namer>>
* <<#sort,sort>>
