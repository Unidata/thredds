...
* <<#baseElements,*Base Catalog Elements*:>>
** <<#catalog,catalog>>
** <<#service,service>>
** <<#dataset,dataset>>
** <<#access,access>>
...
* <<#dlElements,*Digital Library Metadata Elements*:>>
** <<#threddsMetadataGroup,threddsMetadataGroup>>
...
** <<#dateType,dateType>>
** <<#dateTypeFormatted,dateTypeFormatted>>
...
* <<#Dataset_Access_Methods,Dataset Access Methods>>

* <<#constructingURLs,Constructing URLs>>

* <<#datasetClassification,Dataset Classification>>

* <<#webResources,Datasets as Web Resources>>
...
* Catalog <<Example1.0rc8.xml,Example>>

* Catalog XML Schema:

** http://www.unidata.ucar.edu/schemas/thredds/InvCatalog.1.2.xsd[Version 1.2]
...
* <<../reference/DigitalLibraries.adoc,Writing metadata for Digital Libraries>>

* Java API to read catalogs in the http://www.unidata.ucar.edu/software/netcdf-java/[netCDF-Java library].

* <<Changes.adoc,Changes>> since version 1.0.1.

* <<InvCatalogServerSpec.adoc,Server-side catalog specification>>
...
more _<<#service[service]_ elements, followed by zero or more _link:#property,property>>_ elements,
followed by one or more _<<#dataset,dataset>>_ elements. The _base_ is
...
definition of a <<#Dataset_Access_Methods,dataset access method>> to
...
dataset URL (see <<#constructingURLs,constructing URLS>>). The _base_
...
typically one of the _<<#serviceTypes,serviceType>>_ values.
The optional _desc_ attribute allows you to give a human-readable description of the service.

A _service_ element may contain 0 or more _<<#property,property>>_
...
<<#constructingURLs,constructing URLS>> for how the resolved URL is created):
...
*_<<#directDataset,direct>>_* if it contains at least one
<<#Dataset_Access_Methods,dataset access method>>, otherwise it is
just a container for nested datasets, called a
_*<<#collection,collection>>*_ dataset. The name of the dataset
...
<<#threddsMetadataGroup,threddsMetadataGroup>> in any order. These are
followed by 0 or more _<<#access,access>>_ elements, followed by 0 or
...
The _<<#collectionTypes,collectionType>>_ attribute is used to
...
the user. (You can also use a <<#dataType_descrip,_dataType_>> element
...
entries, and is typically placed on <<#collection,collection>>
datasets.
...
<<#Dataset_Access_Methods,data access methods>>. When you have more
than one way to access a dataset, either explicitly define them using
more than one nested <<#access,_access_>> elements, or use a
<<#compoundService,compound service>>.
...
data <<#service,_service_>>. It always refers to the dataset that it
...
<<#constructingURLs,constructing URLs>>). The
_<<#dataFormatType,dataFormat>>_ is important when the
<<#serviceTypes,_serviceType_>> is a bulk transport like _FTP_ or
...
An _access_ element may contain an optional <<#dataSize,_dataSize_>>
...
The <<#XLink,XLink attributeGroup>> allows you to add Xlink
...
elements of both _<<#dataset[dataset]_ and _link:#metadata,metadata>>_
...
A _<<#documentationType,documentation>>_ element contains (or points
...
_<<#metadataElement,metadata>>_ element is a container for
_machine-readable_ information structured in XML. A
_<<#property,property>>_ element is an arbitrary name/value pair.

The next group of elements are used primarily for use in Digital
Libraries. A <<#contributorType,_contributor_>> element is typically a
...
<<#dateTypeEnum,date type enumerations>>. A _keyword_ element is used
for library searches, while a _project_ element specifies what
scientific project the dataset belongs to. Both have type
<<#controlledVocabulary,controlledVocabulary>>, which allows an
...
element use the <<#sourceType,sourceType>> definition.

The next group of elements are used in search services. The
_<<#geospatialCoverageType,geospatialCoverage>>_ element specifies a
lat/lon bounding box for the data. The
_<<#timeCoverageType,timeCoverage>>_ element specifies the range of
dates that the dataset covers. The _<<#variablesType,variables>>_
...
<<#dataType_types_,data type enumeration>> which are intended to map
...
<<#dataFormatType,data format enumeration>>. The _serviceName_ element
...
of allowing for <<#globally_unique_id,globally unique IDs>>. The
...
link:../reference/DigitalLibraries.adoc[how elements are mapped to
...
has an optional <<#docTypeEnum,documentation type>> attribute, such as
...
the <<#metadata,metadata>> element instead.
...
<<#threddsMetadataGroup,threddsMetadataGroup>> in any order, OR it
...
page, use the <<#documentation,documentation>> element instead.
...
values are listed in the <<#metadataType,metadataType>> enumeration.
To use metadata elements from the
<<#threddsMetadataGroup,threddsMetadataGroup>>, do not include the
...
<<#catalog[catalog], <<#dataset,dataset>> or link:#service,service>>
...
This is used by the <<#creator,creator>> and
<<#publisher,publisher>> elements to specify roles of responsibility
...
<<#controlledVocabulary,controlled vocabulary>>. The _contact_ element
...
<<#dateType,date type>> element; 2) by specifying a _start_ element
and a <<#durationType,_duration_>> element; or 3) by specifying an
...
**java.text.SimpleDateFormat**. The <<#dateTypeEnum,values>> of the
...
These describe the data formats, used in an <<#access,access>>
attribute or <<#dataset,dataset>> element, when the service is a bulk
...
These are
the link:../../netcdf-java/reference/FeatureDatasets/Overview.adoc[Feature
...
<<#date,date>> element. These values were taken from the Dublin Core
metadata set.
...
the <<#documentation,documentation>> element.
...
These are the known service types, used in a <<#service,service>>
...
*serviceType* and the <<#dataFormatType,dataFormat>> is intended to be
...
need to also specify the <<#dataFormatType,dataFormat>> for datasets
...
<<#compoundService,Compound Service>> just indicates that the service is composed of other services.
...
the <<#variables,variables>> element. *CF* refers to the
...
<<#compound,Access Methods and Compound Services>> section below. +
...
A query dataset looks a lot like a <<#catalogRef,catalogRef>>, since
...
The reference implementation of THREDDS datasets is the <<../../netcdf-java,netCDF-Java library>>,
...
* <<#access,access>> element; defines a specific service and URL to
access a dataset.
* <<#authority,authority>> : an element of a dataset; can also be
specified as an attribute; used for unique IDs.
* <<#base,service@base>> : base URL is an attribute of a
<<#service,service>> element ; dataset URLs can be reletive to it .
* <<#catalog,catalog>> element; root XML element.
* <<#catalogRef,catalogRef>> element; refers to a nested THREDDS
catalog.
* <<#collectionTypes,collectionTypes>> enumeration, used by
<<#collection,collection>> attribute of dataset.
* <<#contact,contact>> : an attribute used by publisher and creator
elements.
* <<#contributor,contributor>> element, used in
<<#threddsMetadataGroup,threddsMetadataGroup.>>
* <<#controlledVocabulary,controlledVocabulary>> adds an optional
vocabulary attribute to the string-valued element.
* <<#creator,creator>> element, used in
<<#threddsMetadataGroup,threddsMetadataGroup.>>
* <<#dataFormatType,dataFormatTypes>> enumeration, used by
<<#dataFormatType,dataFormat>> element, specifies the file format of
the data.
* <<#dataSize,dataSize>> element; size of the dataset in bytes.
* <<#dataType,dataTypes>> enumeration, used by
<<#dataType,dataType>> attribute of dataset element, specifies the
kind of data in the dataset.
* <<#dataset,dataset>> element; represents a coherent set of data.
* <<#date,date>> element, used in
<<#threddsMetadataGroup,threddsMetadataGroup.>>
* <<#dateType,dateType>> is an attribute or element specifying a date.
* <<#dateTypeFormatted,dateTypeFormatted>> is an element specifying a
date, with optional formatting and type attributes.
* <<#documentation,documentation>> contains human readable information
about the dataset.
* <<#durationType,duration>> a time range.
* <<#expires,expires>> : attribute of catalog, indicates how long the
information is valid.
* <<#geospatialCoverage,geospatialCoverage>> element, defines the
spatial area that the data covers, using
<<#geospatialCoverageType,geospatialCoverageType>>.
* <<#harvest,harvest>>: an attribute of dataset, specifies that the
dataset should be harvested into a Digital library record.
* <<#keyword,keyword>> element, used in
<<#threddsMetadataGroup,threddsMetadataGroup.>>
* <<#metadata,metadata>> element is a container or a pointer to other
metadata elements.
* <<#metadataType,metadataType>> values are the "well known" types
of metadata records, used in a metadata element.
* <<#project,project>> element, used in
<<#threddsMetadataGroup,threddsMetadataGroup.>>
* <<#property,property>> element, specifies name/value pairs.
* <<#publisher,publisher>> element, used in
<<#threddsMetadataGroup,threddsMetadataGroup.>>
* <<#rights,rights>> : a specialized documentation element; what
restrictions are on the use of the dataset.
* <<#service,service>> element defines how and where a dataset is
accessed.
* <<#serviceTypes,serviceTypes>> enumeration, used by serviceType
attribute of <<#service,service>>
* <<#sourceType,sourceType>> is the type of a creator and publisher
element.
* <<#spatialRange,spatialRange>> is a data type used in a
<<#geospatialCoverageType,geospatialCoverage>> element.
* <<#summary,summary>> : a specialized documentation element, a short
description or abstract of the dataset.
* <<#timeCoverage,timeCoverage>> element, defines the time range that
the data covers, using <<#timeCoverageType,timeCoverageType>>.
* <<#threddsMetadataGroup,threddsMetadataGroup>> group of elements,
used in dataset or metadata element.
* <<#variablesType,variables>> element, used in
<<#threddsMetadataGroup,threddsMetadataGroup,>> specifies the
variables (aka _fields_ or __parameters__) available in the dataset.
* <<#variableMap,variableMap>> element is used in
<<#variables,variables>> to link to an external document specifying
the variable names.
* <<#variableNameVocabulary,variableNameVocabulary>> is an enumeration
of known vocabularies for standard variable names, used in the
<<#variables,variables>> element.
* <<#XLink,XLink>> group of attributes; defines a link to another
document.
* <<#zpositive,zpositive>> is an attribute of
<<#geospatialCoverageType,geospatialCoverage>> element, indicating if
