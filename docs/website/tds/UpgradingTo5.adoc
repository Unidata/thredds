...
** *PointFeatureIterator* extends _AutoCloseable, Iterator<PointFeature>_  (deprecate _finish()_)
...
The interface was changed to remove _throws IOException_ and now those are wrapped in _RuntimeException_.
