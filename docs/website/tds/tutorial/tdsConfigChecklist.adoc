...
(link:BasicThreddsConfig_xml.html[more info])
...
(link:../reference/ThreddsConfigXMLFile.html#netcdfCLibrary[more info])
* Configure caches, if required
(link:../reference/ThreddsConfigXMLFile.html#Cache_Locations[more info])
* Configure and add new Viewer links
(link:../reference/Viewers.html[more info])
* Specify other root catalogs, if needed
(link:../reference/ThreddsConfigXMLFile.html#catalogRoots[more info])
* If needed, Restrict Dataset Access using Tomcat
(link:../reference/RestrictedAccess.html[more info])
...
catalog (link:BasicConfigCatalogs.html[more info])
...
* If you have enabled link:TDSMonitoringAndDebugging.html[access
...
* Check the various new logs for errors (link:TroubleShooting.html[more
info])
