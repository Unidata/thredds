...
1.  <<images/fileIcon.jpg,Download and install>> the latest version of
JDK 1.7
...
<<Security.adoc#install,Modify the permissions>> in *$\{tomcat_home}*
to allow this user access to only the necessary parts of the Tomcat
installation.

<<images/fileIcon.jpg,Download and install>> the latest version of
Tomcat 7.
...
* <<Security.adoc#digested,Enable digest passwords>> by commenting out
the *UserDatabaseRealm* and enabling the **MemoryRealm**.
* <<Security.adoc#ssl,Enable SSL>> by uncommenting the SSL Connector
...
* <<TDSMonitoringAndDebugging.adoc#access,Enable access logging>> by
uncommenting the *AccessLogValve* and change the *prefix* and *suffix*
and *pattern* attributes.

<<Security.adoc#digested,Create a password digest>> using the
*$\{tomcat_home}/bin/digest.sh* script.

Make the following modifications to
**$\{tomcat_home}/conf/tomcat-users.xml**:

* Define the roles <<GettingStarted.adoc#grantingAccess,manager-gui>>
and
<<examples/accessingTDSMonitoringAndDebuggingTools.adoc,tdsConfig>>
and
<<examples/accessingTDSMonitoringAndDebuggingTools.adoc,tdsMonitor>>.
* <<GettingStarted.adoc#grantingAccess,Add yourself as a user>> with
...
<<examples/tomcatManagerSSL.adoc,available via SSL>> by adding a
user-data-constraint with a transport-guarantee of CONFIDENTIAL inside
the security-constraint element.

<<Security.adoc#unused,Remove any unused applications>> from
$\{tomcat_home}/webapps.

=== TDS Install

<<GettingStarted.adoc#deploying,Download>> the latest thredds.war
...
in this <<BasicConfigCatalogs.adoc#exampleCatalog,example>>.
...
<<BasicThreddsConfig_xml.adoc#serverInfo,`serverInformation` element>>
* <<AddingServices.adoc,Enable any other optional services>> like WMS
or WCS.

If needed, <<Security.adoc#access,limit access to the TDS>> and create
...
<<Security.adoc#versions,latest versions>> of Java, Tomcat and the
TDS.*

* If you have enabled <<TomcatAndTDSLogs.adoc#access,access logging>>
...
* <<GettingStarted.adoc#deploying,manual installation>>
