...
The records are then grouped by reference time, and a seperate collection is written for each reference time.
The overall dataset is then the collection of all of those single-reference time collections.

This option is reasonable only for small collections, say < 1M records.
