...
period**,** or **none**. Details <<Partitions.adoc,here>>.
...
* <<FeatureCollections.adoc,Feature Collection>> overview
...
5.  The <<CollectionSpecification.adoc,collection specification>>
defines the collection of files that are in this dataset.
6.  The <<Partitions.adoc,partitionType>> is **none**.
...
<<CollectionSpecification.adoc,collection specification>>, or by using
...
<gribConfig> element to name them (see <<GribConfig.adoc,below>> for
details).
...
GDS. Here we correct one such problem (see <<GribConfig.adoc,below>>
for details).
4.  Group renaming as in example 2
5.  Exclude GRIB records that have a time coordinate interval of (0,0)
(see <<GribConfig.adoc,below>> for details).
