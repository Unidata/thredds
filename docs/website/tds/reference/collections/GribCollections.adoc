...
period**,** or **none**. Details link:Partitions.adoc[here].
...
* link:FeatureCollections.adoc[Feature Collection] overview
...
5.  The link:CollectionSpecification.adoc[collection specification]
defines the collection of files that are in this dataset.
6.  The link:Partitions.adoc[partitionType] is **none**.
...
link:CollectionSpecification.adoc[collection specification], or by using
...
<gribConfig> element to name them (see link:GribConfig.adoc[below] for
details).
...
GDS. Here we correct one such problem (see link:GribConfig.adoc[below]
for details).
4.  Group renaming as in example 2
5.  Exclude GRIB records that have a time coordinate interval of (0,0)
(see link:GribConfig.adoc[below] for details).
