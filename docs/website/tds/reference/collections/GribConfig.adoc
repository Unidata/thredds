...
. <<#gdsHash,gdsHash>>: Fix errors in GDS encoding
. <<#gdsName,gdsName>>: Set group names
. <<#pdsHash,pdsHash>>: Control how GRIB records are grouped into variables
. <<#intvFilter,intvFilter>>: control how time intervals are handled
. timeUnitConvert: do not use
. <<#parameter,parameter>>: set miscellaneous values
. <<#gdsName,latestName>>: Rename the latest file dataset
. <<#bestNamer,bestNamer>>: Rename the best file dataset
. <<#datasetTypes,datasetTypes>>: which datasets appear in the TDS catalog:
