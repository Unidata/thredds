...
<4>  *dateFormatMark* (optional): This defines a __<<#dateExtractor,DateExtractor>>__, which is applied to each file in the collection to assign it a
...
it is up to date, depending on  the <<#collectionUpdateType,*_collectionUpdateType_*>>.
...
<1>  **rewrite**: one of the <<#collectionUpdateType,collectionUpdateTypes>>, except for *_never_*. The most useful value is *_test_*.
<2>  **rescan**: uses a http://www.quartz-scheduler.org/docs/tutorials/crontrigger.html[cron expression] to specify when the collection should be
rescanned.
...
name, and _type_ is a <<#collectionUpdateType,collectionUpdateType>>.
...
You must <<#trigger,enable remote triggers>>, and when the dataset changes, send a message to a special URL in the TDS.
