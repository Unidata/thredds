...
* <<FmrcCollection,FMRC Collections>> http://www.unidata.ucar.edu/staff/caron/presentations/FmrcPoster.pdf,poster>>
...
4.  *tdm* element: for GRIB collections, you may use the <<TDM#,TDM>> to do all index updating. This is called __external program updating__.
...
The dataset will be read in at startup time by using the existing indexes which must exist. The <<TDM#,TDM>> will test if its changed once a day
