...
Data Manager (link:TDM.adoc[TDM]) is now available for that purpose.
...
* link:FmrcCollection.adoc[FMRC Collections] http://www.unidata.ucar.edu/staff/caron/presentations/FmrcPoster.pdf[poster]
* link:PointFeatures.adoc[Point Collections]
* link:GribCollections.adoc[GRIB Collections]
* link:GribConfig.adoc[GRIB specific configuration]
* link:GribCollectionFaq.adoc[GRIB Collection FAQs]

Also see:

* link:TDM.adoc[THREDDS Data Manager]
...
<1>  *spec* (required): link:CollectionSpecification.adoc[collection specification] string. In this example, the collection contains all files in the
...
located in each file path, then the link:SimpleDateFormat.adoc[SimpleDateFormat] template *yyyyMMdd_HHmm* is applied to the next characters of the
filename to create a date. A DateExtractor can also be defined in the link:CollectionSpecification.adoc[collection specification] string, but in that
...
<5>  *timePartition* (optional): Currently only used by GRIB collections, see link:Partitions.adoc[here] for more info.
...
If the date is in the filename only, you can use the link:CollectionSpecification.adoc[collection specification] string, aka a **_spec_**:
...
position 17 though 30) and applies the link:SimpleDateFormat.adoc[SimpleDateFormat] _yyyyMMdd_HHmm_ pattern to them.
...
given to link:SimpleDateFormat.adoc[SimpleDateFormat], in this example:
...
You must use the *tdm* element for GRIB collections that change. The link:TDM.adoc[TDM] is a separate process that uses the same configuration
...
.  Triggering is password protected and uses SSH, see link:../RemoteManagement.adoc[enabling Remote Management] to enable SSH.
...
4.  *tdm* element: for GRIB collections, you may use the link:TDM.adoc[TDM] to do all index updating. This is called __external program updating__.
...
The dataset will be read in at startup time by the TDS using the existing indexes, and will be scanned by the link:TDM.adoc[TDM] every 15 minutes,
...
The dataset will be read in at startup time by using the existing indexes which must exist. The link:TDM.adoc[TDM] will test if its changed once a day
...
* link:FCvsAgg.adoc[Feature Collections vs Aggregations]
