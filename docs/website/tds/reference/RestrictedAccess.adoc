...
link:PluggableRestrictedAccess.html[plug in your own] Authentication.
...
link:TomcatSecurity.html#Separate[why]). In this example we are storing
passwords in cleartext, but you are advised to
link:TomcatSecurity.html#Digest[store passwords in digest form].
...
You must link:RemoteManagement.html#SSL[enable Secure Sockets]. See the
link:RemoteManagement.html[TDS Remote Management] guide.

We also recommend that you link:TomcatSecurity.html#Digest[store Digest
Passwords]. See the link:TomcatSecurity.html[Tomcat Security] guide.
...
link:RemoteManagement.html#SSL[Create a new one], and restart Tomcat.
