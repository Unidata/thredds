...
|*timeOffset*|no |Restrict to an offset from the runtime coordinate (timeOffset units same as the dataset), or _first_ or _all_ |Used for 2D time coordinate | all
...
* TimeOffset units must be the same as the dataset.
* Time parameters are only used if timeOffset is not set. It must be explicitly set (ie no default is used). There are 2 cases:
