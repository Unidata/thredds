= NetCDF Subset Service (NCSS) For Grid Data
:linkcss:
:stylesheet: ../../tds.css
...
*_Grid as Point_* request parameters are almost identical to regular NCSS Grid request, but what gets returned is different.
If a netcdf file is requested, it will be written in *_CF / netCDF Discrete Sample Geometry_* format. Other possible formats are the same
as NCSS Point requests.
* Will be timeseries or timeSeriesProfile

[width="100%",cols="20%,10%,30%,30%,10%",options="header",]
|====
| Parameter Name | Required  | Description  | Constraints | default
| *latitude, longitude* | no | Point location. units of degrees_east, degrees_north | Must be within the dataset bounding box |
| *gridAsPoint* | no | make into a gridAsPoint request | _true_ or _false_ | false
| *accept* | no | The returned data format. | _netCDF_, _netCDF4_, _XML_, _CSV_ | _CSV_
|====

* if you specify a point location with the _latitude, longitude_ parameters, the request becomes a gridAsPoint request.
Other horizontal subsetting parameters are ignored.
