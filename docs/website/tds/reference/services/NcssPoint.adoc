...
|*time* |no | Time as a link:{w3cDate}[W3C date] or _present_ or _all_ | |_present_
|*time_window* |no | Use with *time*: how close a match; udunit time or link:{w3cDuration}[W3C time duration] | | _1 hour_
...
Station datasets only. use the station closest to the given latitude, longitude. Example:
