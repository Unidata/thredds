...
<<#DatasetDescription,Dataset Description>> XML document, which describes the dataset in enough detail to enable a programmatic client to form valid
data requests.
...
(<<../files/gridsDataset.xml,offline>> example)
...
(<<../files/stationsDataset.xml,offline>> example)
...
(<<../files/pointDataset.xml,offline>> example) 
