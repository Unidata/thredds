...
<<ThreddsConfigXMLFile#FileCache,TDS configuration values>>, this
...
<<ThreddsConfigXMLFile#FileCache,threddsConfig.xml>> file.
...
Each NetcdfFile object encapsulates a file. NcML aggregations are
...
files per second, it shouldn't get too much bigger. But leave some
