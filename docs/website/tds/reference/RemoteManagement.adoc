...
<<TomcatSecurity.adoc,Tomcat/TDS Security guidelines>>.

Follow the <<../tutorial/Checklist.adoc,checklist>> for more concise
...
the <<TomcatSecurity.adoc,Tomcat/TDS Security guidelines>>, using the
...
<<#JRE_keystore,Install>> the remote host’s certificate into the JRE
certificate store.
