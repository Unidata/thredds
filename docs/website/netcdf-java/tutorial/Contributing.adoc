...
.. `cd thredds`
. Create a branch for the changes that you will make based on `4.6.x`.
.. `git checkout -b <branch_name> 4.6.x`
.. For instance: `git checkout -b fix_radar_iosp 4.6.x`
. Make changes to the source code. See link:SourceCodeBuild.html[this tutorial] for help building THREDDS.
