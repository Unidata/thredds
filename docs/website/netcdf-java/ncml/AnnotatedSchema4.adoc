...
6.  Nested netcdf datasets can be implictly specified with a *scan*
element.
...
java.text.link:#SimpleDateFormat[SimpleDateFormat] string. The number of
...
ncml needs to operate on the datset after the CoordSysBuilder has
augmented it. Otherwise, you should not enhance.
14. A specialized scanFmrc element can be used for a
_*link:FmrcAggregation.html#forecastModelRunSingleCollection[forecastModelRunSingleCollection]*_
...
java.text.link:#SimpleDateFormat[SimpleDateFormat] string before or
