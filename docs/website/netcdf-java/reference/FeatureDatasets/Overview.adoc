...
* <<FMRC.adoc,Forecast Model Run Collection (FMRC)>>: Gridded data with two time coordinates, _Run Time_ (aka _Reference Time_)
and _Forecast Time_ (aka _Valid Time_)
* <<../../tutorial/RadialDatatype.adoc,Radial Dataset>>: uses polar coordinates (elevation, azimuth, distance), for example scanning radars, lidars. +
