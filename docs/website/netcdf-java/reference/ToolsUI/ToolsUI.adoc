...
The NetCDF-Java library can read data from a wide variety of link:../formats/FileTypes.adoc[scientific data formats].
...
=== Writer
The Writer tab takes a dataset in link:../formats/FileTypes.adoc[any format that NetCDF-Java can understand] and writes
...
Note: to write to NetCDF-4, you must have the link:../netcdf4Clibrary.adoc[C library loaded].
...
link:../FeatureDatasets/Overview.adoc[scientific feature types] that a CDM dataset can contain.
...
link:../FeatureDatasets/PointFeatures.adoc[point features] or not. Simply try to open the dataset, the same way you did
...
it's easy to modify the dataset using the link:../../ncml/Tutorial.adoc[powers of NcML] and write the document out to
disk.
