...
*NetcdfFile* can open *HTTP remote files*, <<HTTPservice.adoc,served over HTTP>>, for example:
...
<<Caching.adoc,cache directory>> defined by *ucar.nc2.util.DiskCache.*
...
NetcdfDataset can open *_<<stream/CdmRemote.adoc,CDM Remote>>_* datasets, with the form
...
FeatureDatasetFactory creates <<FeatureDatasets/Overview.adoc,Feature Datasets>> for Coverages (Grids),
...
FeatureDatasetFactoryManager can open *_<<stream/CdmrFeature.adoc,CdmRemote Feature Datasets>>_*, which have the form
