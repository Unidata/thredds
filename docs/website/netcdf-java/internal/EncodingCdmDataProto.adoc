...
<4> *vlens* has section.size array lengths
...
. almost identical with Data, perhaps can use instead
...
. section is not used
. nelems is used to specify how many StructureData objects are in the Data message.
...
I think of this as column oriented. Potentially more efficient. (ArrayStructureMA instead of ArrayStructureW)
