...
<1> *primarray* has nelems * sizeof(dataType) bytes, turn into multidim array of primitives with section info and bigend
<2> *stringdata* has nelems strings, turn into multidim array of String with section info
<3> *structdata* has nelems StructureData objects, turn into multidim array of StructureData with section info and bigend
<4> *vlens* has section.size array lengths; section does not include the last (vlen) dimension
<5> *opaquedata* has nelems opaque objects, turn into multidim array of Opaque with section info

Issues

. Cant use _oneof_ because it doesnt allow repeated
. not trying to be backwards compatible - not sure if its possible
...
. almost identical with Data, perhaps can use Data instead of MmberData?
. could get rid of ArrayStructureDataCol and use repeated MemberData memberData in Data ??
...
. section is not used
. nelems is used to specify how many StructureData objects are in the Data message. This is (*)
...
** Clen is a List, non-vlen is an array
...
These can only make other vlens (last dimension may be a vlen)
...
----
  ...
  string s.fld2(*, 12);
  float s.inner.fld3(*,99);
  long s.inner.fld4(*,99,2,*);
----

so i think not a problem