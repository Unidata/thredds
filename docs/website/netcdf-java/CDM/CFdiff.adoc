...
The units attribute takes a string value formatted as per the recommendations in the
[http://cfconventions.org/Data/cf-conventions/cf-conventions-1.7/build/cf-conventions.html#udunits[Udunits package]].
...
This allows the CDM to process the
http://cfconventions.org/Data/cf-conventions/cf-conventions-1.7/build/cf-conventions.html#calendar[CF calendar attribute].
See link:CalendarDateTime.adoc[CDM Calendar Dates] for details.
...
link:../reference/StandardCoordinateTransforms.adoc[standard grid_mapping] that specifies the projection, the CDM uses the projection
...
* CDM: the implementation has some differences, described link:../reference/FeatureDatasets/CFpointImplement.adoc[here].
