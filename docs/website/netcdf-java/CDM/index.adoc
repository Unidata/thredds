...
link:../reference/formats/FileTypes.adoc[file formats] besides netCDF.
...
only be found by reading the data. A link:VariableLengthData.adoc[variable length Dimension] cannot be
...
*link:DataType.adoc[DataType]* describes the possible types of data:
...
See link:Netcdf4.adoc[here] for detailed mapping between the NetCDF-4
and CDM data models.
...
See link:Opendap.adoc[here] for more details on OPeNDAP processing.
...
A *_Coordinate System_* has one or more CoordinateAxis, and zero or more
CoordinateTransforms.

A *_CoordinateAxis_* is a subtype of Variable, and is optionally
...
The *_AxisType_* enumerations are specific to the case of georeferencing
...
See link:../reference/FeatureDatasets/CFpointImplement.adoc[CF point data]
...
build on the CDM have a single API to work with.
...
There is not a similarly elegant way to specify a multidimensional CoordinateAxis, and so various attribute
conventions have sprung up, for example the CF Convention simply lists the CoordinateAxis variables (arguably as simple a solution as is possible):
...
link:../reference/CoordinateAttributes.adoc[underscore Coordinate]
...
* link:../reference/FeatureDatasets/Overview.adoc[FeatureDataset APIs]
* link:../reference/FeatureDatasets/PointFeatures.adoc[Point Feature APIs]
* http://cfconventions.org/Data/cf-conventions/cf-conventions-1.6/build/cf-conventions.html#discrete-sampling-geometries[CF Discrete Sampling Geometries Conventions]
